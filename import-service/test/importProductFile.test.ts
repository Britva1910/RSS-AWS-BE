...
describe('importProductsFile Lambda', () => {
    ...
    it('should return a 400 error when no file name is provided', async () => {
        ...
        expect(JSON.parse(response.body).message).toBe('Missing file name in query parameters');
    });
    ...
    it('should return a 500 error if getSignedUrl throws an error', async () => {
        ...
        (getSignedUrl as jest.Mock) = jest.fn().mockRejectedValue(new Error('Some error'));

        const response = await handler(event);

        expect(response.statusCode).toBe(500);
        expect(JSON.parse(response.body).message).toBe('Could not generate signed URL');
    });
});
